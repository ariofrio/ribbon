...
- Capturing needs the Node in `.nvmrc`, `npx playwright install chromium`, and `npm ci --prefix scripts/screenshots`, and runs on macOS or Linux alike. That last one is bb: the harness renders against the `bb-app` package pinned beside it, not the desktop app, so the bb these shots are taken against is a line in a manifest the shot digest already hashes rather than whatever the machine has installed — and bumping it reports every screenshot stale, which is what a new bb does to them. `npm run check:screenshots` needs none of it, and CI runs it to report a screenshot whose plugin changed after it was captured.
- CI recaptures a stale shot itself, in the container pinned in `.github/workflows/screenshots.yml`, and commits the result back to the branch. Capture locally when it is quicker to look at, but that container is the one whose output is authoritative, and its Chromium is pinned by the `playwright` version in `package-lock.json` — bump one and the image tag moves with it.
