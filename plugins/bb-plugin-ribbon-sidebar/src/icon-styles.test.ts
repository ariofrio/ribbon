...
describe("iconStyles", () => {
  const ruleFor = (fallback: IconFallback) => {
    const match = new RegExp(
      `\\[data-ribbon-sidebar-icon="${fallback}"\\]\\{(.+?)\\}`,
      "u",
    ).exec(iconStyles());
    return match?.[1];
  };

  it("prefers what somebody picked, and falls back to this plugin's own glyph", () => {
...
describe("publishIconStyles", () => {
  it("puts one sheet in, and takes it back out", () => {
    ...
    const style = document.head.querySelector("style[data-ribbon-sidebar-icons]");
