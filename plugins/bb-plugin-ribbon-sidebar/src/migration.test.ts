...
describe("Thread stages migration", () => {
  ...
  it("imports and verifies each snapshot before acknowledging its exact revision", async () => {
    ...
    expect(store.getPlacement({ groupingKey, threadId: "thread-a" })).toMatchObject({
      ok: true,
      value: {
        placement: {
          groupId: "Active",
  ...
  it("does not rewrite an already imported source revision", async () => {
    ...
    const client = {
      ...
      acknowledgePlacementMigrationV1: vi.fn(async () => ({ transferred: true })),
    };

    expect(await migrateThreadStages(store, client)).toMatchObject({ imported: true });
    const afterImport = store.getPlacement({ groupingKey, threadId: "thread-a" });
    expect(await migrateThreadStages(store, client)).toMatchObject({ imported: false });
    expect(store.getPlacement({ groupingKey, threadId: "thread-a" })).toEqual(afterImport);
  });
});
