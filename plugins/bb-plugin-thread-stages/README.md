...
selected carries that selection into bb's New thread composer. A fork without
an explicit section inherits the nearest section on its fork source's ancestor
chain; an ordinary CLI-spawned thread remains Unorganized. When a thread is
unparented, it keeps the nearest section from its former parent hierarchy.
Reparenting leaves its own section unchanged so it inherits from its new
parent.
